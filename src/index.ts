import type { ISoundManifest } from "./audioplayer.ts";
...
async function main() {
  ...
  const soundManifest: ISoundManifest = {
    wind: {
      name: "Wind",
  ...
  const ctrlArea = document.querySelector<HTMLDivElement>("#controls")!
  ...
  for (let k in channels) {
    ...
    const ipt = el.querySelector<HTMLInputElement>(`#${k}`)!;
  ...
  const playBut = document.querySelector<HTMLButtonElement>("#play")!;
